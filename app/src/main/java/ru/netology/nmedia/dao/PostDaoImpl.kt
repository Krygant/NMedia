...
class PostDaoImpl(private val db: SQLiteDatabase) : PostDao {
    ...
    override fun save(post: Post): Post {
        val values = ContentValues().apply {
            ...
            put(PostColumns.COLUMN_VIDEO, "NO")
        }
    ...
    override fun getPlayVideo(id: Long): String? {
        val projection = arrayOf(PostColumns.COLUMN_VIDEO)
        val selection = "${PostColumns.COLUMN_ID}=?"
        val selectionArgs = arrayOf(id.toString())

        val cursor: Cursor? = db.query(PostColumns.TABLE, projection, selection, selectionArgs, null, null, null)

        return try {
            if (cursor != null && cursor.moveToFirst()) {
                cursor.getString(cursor.getColumnIndex(PostColumns.COLUMN_VIDEO) ?: 0)
            } else {
                null
            }
        } finally {
